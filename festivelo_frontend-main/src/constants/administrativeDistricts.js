...
Object.entries(ADMINISTRATIVE_DISTRICTS).forEach(([city, districts]) => {
  DISTRICT_MAP[city] = city;
  districts.forEach(district => {
    const fullName = `${city} ${district}`;
    DISTRICT_MAP[fullName] = city;
  });
});

export const findRegion = (keyword) => {
  const normalizedKeyword = keyword.trim().replace(/\s+/g, ' ');
  if (DISTRICT_MAP[normalizedKeyword]) {
    return {
      fullName: normalizedKeyword,
      city: DISTRICT_MAP[normalizedKeyword]
    };
  }

  const matchingKeys = Object.keys(DISTRICT_MAP).filter(key => 
    key.toLowerCase().includes(normalizedKeyword.toLowerCase())
  );

  if (matchingKeys.length > 0) {
    const shortestMatch = matchingKeys.reduce((a, b) => 
      a.length <= b.length ? a : b
    );
    return {
      fullName: shortestMatch,
      city: DISTRICT_MAP[shortestMatch]
    };
  }
